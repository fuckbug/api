...
import (
	...
	"github.com/fuckbug/api/pkg/response"
...
func (h *logHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	id := vars["id"]
	if id == "" {
		response.RespondWithPlainError(w, http.StatusBadRequest, "id is required")
		return
	}

	entity, err := h.service.GetByID(r.Context(), id)
	if err != nil {
		response.RespondWithPlainError(w, http.StatusNotFound, err.Error())
		return
	}

	response.RespondWithJSON(w, http.StatusOK, entity)
}
...
func (h *logHandler) GetAll(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.RespondWithPlainError(w, http.StatusInternalServerError, err.Error())
		return
	}

	response.RespondWithJSON(w, http.StatusOK, response.NewListResponse(totalCount, logs))
}
...
func (h *logHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err := response.DecodeRequest(w, r, &req); err != nil {
		return
	}

	if err := h.validate.Struct(req); err != nil {
		response.HandleValidatorError(w, err)
		return
	}
	...
	response.RespondWithJSON(w, http.StatusCreated, entity)
}
